...
fn nanami_main() -> libnanami::NanamiResult {
    ...
    libnanami::print!("[alter] service registered: alter-linux\n");
        ...
        pending = match event {
            ...
            ServiceEvent::Notification { identifier, .. } => {
                ...
                if identifier & nanami_services::net::NET_NOTIFICATION_RX != 0 {
                    linux::wake_network_waiters(&mut runtime);
                }
                if identifier & nanami_services::input::INPUT_NOTIFICATION_IDENTIFIER != 0 {
                    linux::wake_device_readers(&mut runtime);
                }
