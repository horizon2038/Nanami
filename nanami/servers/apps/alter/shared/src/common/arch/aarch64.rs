...
pub const PLATFORM: &[u8] = b"aarch64";
pub const PLATFORM_NUL: &[u8] = b"aarch64\0";
pub const UNAME_MACHINE: &[u8] = b"aarch64";
pub const PROC_VERSION: &[u8] = b"Linux version 6.1.0-alter (Nanami/A9N) aarch64\n";
pub const CPU_INFO: &[u8] =
    b"processor\t: 0\nmodel name\t: Alter virtual AArch64 processor\nFeatures\t: fp asimd\n";
...
pub unsafe fn write_stat_buffer(base: Word, inode: Word, size: Word, mode: Word, rdev: Word) {
    unsafe {
        ...
        write_u32(base + 16, mode as u32);
        write_u32(base + 20, 1);
        write_u32(base + 24, 0);
        write_u32(base + 28, 0);
        write_u64(base + 32, rdev);
        write_u64(base + 48, size);
        write_u32(base + 56, 4096);
