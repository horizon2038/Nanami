...
const REG_RAX: Word = 0;
const REG_RDX: Word = 3;
const REG_RDI: Word = 4;
const REG_RSI: Word = 5;
const REG_R8: Word = 7;
const REG_R9: Word = 8;
const REG_R10: Word = 9;
const REG_GPR_END: Word = 15;
pub const REG_PC: Word = 15;
const REG_RIP: Word = REG_PC;
const REG_RFLAGS: Word = 17;
pub const REG_RSP: Word = 18;
const REG_GS_BASE: Word = 20;
pub const REG_FS_BASE: Word = 21;

const REGISTER_COUNT_FOR_SYSCALL: Word = 16;
const REGISTER_COUNT_FOR_FREEBSD_SYSCALL: Word = 18;
const REGISTER_COUNT_FULL: Word = 22;
...
pub fn write_exec_registers(
    ...
    a9n_abi::arch::process_control_block::read_register(pcb, REGISTER_COUNT_FULL)
        .map_err(|_| ())?;
    let ipc = a9n_abi::arch::ipc_buffer::get_ipc_buffer();
    // Preserve the CS/SS selectors initialized by A9N. Clearing the entire
    // context here makes the first interrupt return to a null user segment.
    for register in 0..REG_GPR_END {
