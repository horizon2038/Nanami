...
fn handle_read_direct(runtime: &mut Runtime, request: ServiceRequest) -> (Word, Word, Word) {
    handle_read_with_mode(runtime, request, true)
}

fn handle_read_with_mode(
    runtime: &mut Runtime,
...
fn handle_write(runtime: &mut Runtime, request: ServiceRequest) -> (Word, Word, Word) {
    ...
    if input_offset.saturating_add(len) > runtime.sessions[index].shm_size as usize {
        return (libnanami::OS_RESPONSE_INVALID_ARGUMENT, 0, 0);
    }
    let open_index = runtime.sessions[index].fds[fd].open_file;
    if open_index >= runtime.open_files.len() || !runtime.open_files[open_index].active {
    ...
    match entry.kind {
        ...
        FdKind::Regular => {
            ...
            unsafe {
                ptr::copy_nonoverlapping(
                    (runtime.sessions[index].shm_local as usize + input_offset) as *const u8,
                    (runtime.vfs_shm as usize + VFS_IO_OFFSET) as *mut u8,
                    chunk,
                );
            }
