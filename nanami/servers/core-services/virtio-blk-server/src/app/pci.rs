...
pub(crate) fn configure_pci_command_for_intx(
    ...
    libnanami::print!("[virtio-blk] pci cmd before=");
    ...
    libnanami::print!("[virtio-blk] pci cmd after =");
...
pub(crate) fn disable_pci_msi_capabilities(
    ...
    if (status & PCI_STATUS_CAP_LIST) == 0 {
        libnanami::print!("[virtio-blk] pci caps: none\n");
        return Ok(());
    }
    ...
    while cap >= 0x40 && guard < 48 {
        ...
        if cap_id == PCI_CAP_ID_MSI {
            ...
            libnanami::print!("[virtio-blk] pci msi ctrl=");
            ...
            libnanami::print!("[virtio-blk] pci msix ctrl=");
            libnanami::print!("{:#x}", ctrl);
            libnanami::print!(" -> ");
            libnanami::print!("{:#x}", next);
            libnanami::print!("\n");
...
fn probe_virtio_at(
    ...
    if vendor_id != VIRTIO_VENDOR_ID
        || !(device_id == VIRTIO_BLK_DEVICE_ID_LEGACY || device_id == VIRTIO_BLK_DEVICE_ID_MODERN)
    {
        return Ok(None);
    }
...
pub(crate) fn resolve_irq_number(
    ...
    if found.irq_pin >= 1 && found.irq_pin <= 4 {
        ...
        if let Some(routed_irq) = read_piix3_pirq_route(io_desc, pirq_index)? {
            libnanami::print!("[virtio-blk] irq routing: PIRQ");
...
pub(crate) fn scan_virtio_blk(io_desc: Word) -> Result<VirtioPciDevice, RequestError> {
    let known = [
        (0u8, 4u8, 0u8),
        (0u8, 5u8, 0u8),
        (0u8, 3u8, 0u8),
